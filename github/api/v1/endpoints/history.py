    ...
    try:
        ...
        items = [
            HistoryItem(
                ...
                has_market_radar=bool(item.get("has_market_radar", False)),
            )
...
@router.post(
    ...
    responses={
        200: {"description": "刷新并返回今日A股持仓决策仪表盘"},
        500: {"description": "服务器错误", "model": ErrorResponse},
    },
    summary="刷新今日A股持仓决策仪表盘",
    description="联网刷新当天 market_radar，保存为历史记录，并返回可直接展示的 report。",
)
def refresh_today_dashboard(
    db_manager: DatabaseManager = Depends(get_database_manager),
) -> AnalysisReport:
    try:
        history_id, _artifact = save_today_market_dashboard(db_manager)
        if not history_id:
            raise RuntimeError("today dashboard history save returned empty id")
        return get_history_detail(str(history_id), db_manager=db_manager)
    except HTTPException:
        raise
    except Exception as e:
        logger.error("刷新今日仪表盘失败: %s", e, exc_info=True)
        raise HTTPException(
            status_code=500,
            detail={
                "error": "internal_error",
                "message": f"刷新今日仪表盘失败: {str(e)}",
            },
        )
...
def get_history_detail(
    ...
    try:
        ...
        details = ReportDetails(
            ...
            kline_chart=result.get("kline_chart"),
        )

...
@router.post(
    ...
    response_model=PortfolioScreenshotImportParseResponse,
    responses={400: {"model": ErrorResponse}, 500: {"model": ErrorResponse}},
    summary="Parse broker holding screenshot into editable portfolio preview",
)
def parse_screenshot_import(
    file: UploadFile = File(...),
    broker_hint: Optional[str] = Form(None),
    screenshot_date: Optional[date] = Form(None),
) -> PortfolioScreenshotImportParseResponse:
    importer = PortfolioImportService()
    try:
        content = file.file.read()
        parsed = importer.parse_screenshot_import(
            content=content,
            mime_type=file.content_type or "image/jpeg",
            broker_hint=broker_hint,
            screenshot_date=screenshot_date,
        )
        return PortfolioScreenshotImportParseResponse(**parsed)
    except ValueError as exc:
        raise _bad_request(exc)
    except Exception as exc:
        raise _internal_error("Parse screenshot import failed", exc)


@router.post(
    "/imports/screenshot/commit",
    response_model=PortfolioScreenshotImportCommitResponse,
    responses={400: {"model": ErrorResponse}, 422: {"model": ErrorResponse}, 500: {"model": ErrorResponse}},
    summary="Commit confirmed screenshot portfolio calibration",
)
def commit_screenshot_import(
    request: PortfolioScreenshotImportCommitRequest,
) -> PortfolioScreenshotImportCommitResponse:
    importer = PortfolioImportService()
    try:
        result = importer.commit_screenshot_import(
            account_id=request.account_id,
        ...
        return PortfolioScreenshotImportCommitResponse(**result)
    except ValueError as exc:
        raise _bad_request(exc)
    except Exception as exc:
        raise _internal_error("Commit screenshot import failed", exc)

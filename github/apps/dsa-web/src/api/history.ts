...
export const historyApi = {
  ...
  /**
   * 刷新今日A股持仓决策仪表盘，并返回可直接展示的报告详情。
   */
  refreshTodayDashboard: async (): Promise<AnalysisReport> => {
    const response = await apiClient.post<Record<string, unknown>>('/api/v1/history/today-dashboard/refresh');
    return toCamelCase<AnalysisReport>(response.data);
  },

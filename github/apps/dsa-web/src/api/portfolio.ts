...
import type {
  ...
  PortfolioScreenshotImportCommitRequest,
...
const SCREENSHOT_IMPORT_TIMEOUT_MS = 180000;
...
export const portfolioApi = {
  ...
  async parseScreenshotImport(
    file: File,
    options: { accountId?: number; brokerHint?: string; screenshotDate?: string } = {},
  ): Promise<PortfolioScreenshotImportParseResponse> {
    const formData = new FormData();
    if (options.accountId != null) {
      formData.append('account_id', String(options.accountId));
    }
    if (options.brokerHint) {
      formData.append('broker_hint', options.brokerHint);
    }
    if (options.screenshotDate) {
      formData.append('screenshot_date', options.screenshotDate);
    }
    formData.append('file', file);
    const response = await apiClient.post<Record<string, unknown>>(
      ...
      {
        headers: { 'Content-Type': 'multipart/form-data' },
        timeout: SCREENSHOT_IMPORT_TIMEOUT_MS,
      },
    );
    return toCamelCase<PortfolioScreenshotImportParseResponse>(response.data);
  },

  async commitScreenshotImport(
    payload: PortfolioScreenshotImportCommitRequest,
  ): Promise<PortfolioScreenshotImportCommitResponse> {
    const response = await apiClient.post<Record<string, unknown>>(
      '/api/v1/portfolio/imports/screenshot/commit',
      {
        account_id: payload.accountId,
        ...
        positions: payload.positions.map((position) => ({
          ...
          field_confidence: position.fieldConfidence,
          warnings: position.warnings,
        })),
      },
    );
    return toCamelCase<PortfolioScreenshotImportCommitResponse>(response.data);
  },
};

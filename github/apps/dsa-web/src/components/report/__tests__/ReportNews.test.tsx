...
describe('ReportNews', () => {
  ...
  it('hides the news panel when no news exists', async () => {
    vi.mocked(historyApi.getNews).mockResolvedValue({
      total: 0,
      items: [],
    });
    ...
    expect(screen.queryByText('暂无相关资讯')).not.toBeInTheDocument();
  });

  it('hides the localized news panel when no news exists', async () => {
    vi.mocked(historyApi.getNews).mockResolvedValue({
      total: 0,
      items: [],
    });
    ...
    expect(screen.queryByText('No related news')).not.toBeInTheDocument();
  });

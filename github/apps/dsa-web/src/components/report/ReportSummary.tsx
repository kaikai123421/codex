...
import { ReportKlineChart } from './ReportKlineChart';
...
export const ReportSummary: React.FC<ReportSummaryProps> = ({
  ...
  const normalizedStockCode = (meta.stockCode || '').trim().toUpperCase();
  const isPortfolioDashboard = normalizedStockCode === 'PORTFOLIO';
  ...
  return (
    <div className="space-y-5 pb-8 animate-fade-in">
      <PositionDecisionDashboard report={report} />
      ...
      {/* K线确认区：放在个股首屏之后，用真实K线/BBI验证结论 */}
      {!isPortfolioDashboard && (
        <ReportKlineChart details={details} strategy={strategy} language={reportLanguage} />
      )}

      {/* 策略点位区 */}
      {!isPortfolioDashboard && <ReportStrategy strategy={strategy} language={reportLanguage} />}

      {/* 资讯区 */}
      {!isPortfolioDashboard && <ReportNews recordId={recordId} limit={8} language={reportLanguage} />}

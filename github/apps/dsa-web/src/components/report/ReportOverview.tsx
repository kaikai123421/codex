...
const isPortfolioReportCode = (stockCode?: string | null): boolean =>
  (stockCode || '').trim().toUpperCase() === 'PORTFOLIO';
...
export const ReportOverview: React.FC<ReportOverviewProps> = ({
  ...
  const shouldShowWatchlist = Boolean(watchlist)
    && meta.reportType !== 'market_review'
    && !isPortfolioReportCode(meta.stockCode);
  const watchlistMessage = watchlist?.actionMessage?.trim();
  const shouldShowWatchlistMessage = Boolean(watchlistMessage)
    && !/(失败|失敗|fail|failed|error)/i.test(watchlistMessage || '');
          ...
          {shouldShowWatchlist && watchlist && (
            <Card variant="bordered" padding="sm" className="home-panel-card">
              <div className="text-center space-y-3">
                ...
                {shouldShowWatchlistMessage && watchlistMessage && (
                  <p className="text-[11px] text-secondary-text animate-in fade-in">{watchlistMessage}</p>
                )}
              </div>
            </Card>
          )}

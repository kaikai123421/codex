...
export const StockBarItemComponent: React.FC<StockBarItemProps> = ({
  ...
  const normalizedStockCode = item.stockCode.trim().toUpperCase();
  const displayStockCode = isMarketReview || normalizedStockCode === 'MARKET'
    ? null
    : normalizedStockCode === 'PORTFOLIO'
      ? (language === 'zh' ? '组合' : 'Portfolio')
      : item.stockCode;
      ...
      <div className="relative z-10 flex items-center gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2" data-testid="history-card-meta">
            ...
            {phaseLabel ? (
              <>
                {displayStockCode ? <span className="w-1 h-1 rounded-full bg-subtle-hover" /> : null}

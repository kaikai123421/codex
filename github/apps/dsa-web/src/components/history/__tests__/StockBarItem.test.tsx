...
describe('StockBarItemComponent', () => {
  ...
  it('keeps portfolio records free of internal code and timestamp clutter', () => {
    render(
      <StockBarItemComponent
        item={{
          ...issue1600Item,
          ...
          stockName: 'A股持仓组合',
          analysisCount: 47,
          lastAnalysisTime: '2026-07-03T09:27:00Z',
        }}
        isViewing={false}
        onClick={vi.fn()}
      />,
    ...
    const meta = screen.getByTestId('history-card-meta');
    expect(within(meta).queryByText('PORTFOLIO')).not.toBeInTheDocument();
    expect(within(meta).queryByText(/2026/)).not.toBeInTheDocument();
    expect(within(meta).queryByText(/47/)).not.toBeInTheDocument();
    expect(within(meta).getByText('组合')).toBeVisible();
  });
});

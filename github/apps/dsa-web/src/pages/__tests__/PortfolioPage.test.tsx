...
const {
  ...
  parseScreenshotImport: vi.fn(),
  commitScreenshotImport: vi.fn(),
  createAccount: vi.fn(),
  deleteAccount: vi.fn(),
...
describe('PortfolioPage FX refresh', () => {
  beforeEach(() => {
    ...
    parseScreenshotImport.mockResolvedValue({
      suggestedAccountName: '广发证券截图账户',
      brokerHint: 'gf',
      screenshotDate: '2026-07-02',
      ...
      positions: [
        {
          ...
          lastPrice: 9.1,
          marketValue: 5460,
          unrealizedPnl: 2645.91,
          unrealizedPnlPct: 94.02,
          confidence: 0.96,
          fieldConfidence: { symbol: 0.99, quantity: 0.98, avgCost: 0.96 },
          warnings: [],
        },
      ],
      warnings: [],
    ...
    commitScreenshotImport.mockResolvedValue({
      accountId: 1,
      recordCount: 2,
      insertedCount: 2,
      duplicateCount: 0,
      ...
      dryRun: false,
      errors: [],
      cashCalibrationAmount: 13710.04,
      snapshot: { totalCash: 10896.04, totalMarketValue: 5460, totalEquity: 16356.04, positionCount: 1 },
    });
  ...
  it('imports a holding screenshot after editable preview confirmation', async () => {
    render(<PortfolioPage />);

    await waitForInitialLoad();

    const file = new File(['fake'], 'position.png', { type: 'image/png' });
    const input = screen.getByLabelText('上传持仓截图') as HTMLInputElement;
    fireEvent.change(input, { target: { files: [file] } });
    fireEvent.click(screen.getByRole('button', { name: '开始识别' }));

    await waitFor(() => expect(parseScreenshotImport).toHaveBeenCalledWith(file, {
      accountId: 1,
      brokerHint: 'gf',
      screenshotDate: expect.any(String),
    }));
    expect(await screen.findByDisplayValue('000725')).toBeInTheDocument();

    fireEvent.click(screen.getByRole('button', { name: '确认写入' }));

    await waitFor(() => expect(commitScreenshotImport).toHaveBeenCalledTimes(1));
    expect(commitScreenshotImport).toHaveBeenCalledWith(expect.objectContaining({
      accountId: 1,
      totalCash: 10896.04,
      positions: [expect.objectContaining({ symbol: '000725', quantity: 600, avgCost: 4.69 })],
    }));
    expect(await screen.findByText(/截图导入完成/)).toBeInTheDocument();

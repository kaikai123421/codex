...
import type {
  ...
  PortfolioScreenshotImportCommitResponse,
  PortfolioScreenshotImportParseResponse,
  PortfolioScreenshotImportPosition,
  PortfolioSide,
...
function getScreenshotFieldConfidence(
  position: PortfolioScreenshotImportPosition,
  field: keyof PortfolioScreenshotImportPosition,
): number {
  return Number(position.fieldConfidence?.[String(field)] ?? position.confidence ?? 0);
}

function isScreenshotFieldWeak(
  position: PortfolioScreenshotImportPosition,
  field: keyof PortfolioScreenshotImportPosition,
): boolean {
  return getScreenshotFieldConfidence(position, field) < 0.8;
}
...
const PortfolioPage: React.FC = () => {
  ...
  const [screenshotBrokerHint, setScreenshotBrokerHint] = useState('gf');
  const [screenshotDate, setScreenshotDate] = useState(getTodayIso());
  ...
  const [screenshotParseResult, setScreenshotParseResult] = useState<PortfolioScreenshotImportParseResponse | null>(null);
  const [screenshotCommitResult, setScreenshotCommitResult] = useState<PortfolioScreenshotImportCommitResponse | null>(null);
  const [screenshotPositions, setScreenshotPositions] = useState<PortfolioScreenshotImportPosition[]>([]);
  const [screenshotImportWarning, setScreenshotImportWarning] = useState<string | null>(null);
  ...
  const screenshotTargetAccount = writableAccount ?? (accounts.length === 1 ? accounts[0] : undefined);
  const screenshotTargetAccountId = screenshotTargetAccount?.id;
  ...
  const handleParseScreenshot = async () => {
    ...
    try {
      ...
      setScreenshotImportWarning(null);
      setScreenshotCommitResult(null);
      const parsed = await portfolioApi.parseScreenshotImport(screenshotFile, {
        accountId: screenshotTargetAccountId,
        brokerHint: screenshotBrokerHint || undefined,
        screenshotDate: screenshotDate || undefined,
      });
      setScreenshotParseResult(parsed);
      setScreenshotPositions(parsed.positions || []);
      if (!hasAccounts && parsed.suggestedAccountName) {
        setAccountForm((prev) => ({
          ...prev,
          name: prev.name || parsed.suggestedAccountName,
          broker: prev.broker || parsed.brokerHint || 'GF',
          market: 'cn',
          baseCurrency: 'CNY',
        }));
  ...
  const updateScreenshotPosition = (
    ...
    setScreenshotPositions((prev) => prev.map((item, itemIndex) => {
  ...
  const hasInvalidScreenshotPositions = screenshotPositions.some((item) => (
    !String(item.symbol || '').trim()
    || Number(item.quantity || 0) <= 0
    || Number(item.avgCost || 0) <= 0
  ));

  const handleCommitScreenshot = async () => {
    if (!screenshotTargetAccountId) {
      setScreenshotImportWarning('请先创建或选择一个具体账户，再确认写入截图持仓。');
      return;
    }
    if (hasInvalidScreenshotPositions || screenshotPositions.length === 0) {
      setScreenshotImportWarning('代码、数量、成本价是必填项；低置信度字段请确认后再写入。');
      return;
    }
    try {
      setScreenshotCommitting(true);
      setScreenshotImportWarning(null);
      const committed = await portfolioApi.commitScreenshotImport({
        accountId: screenshotTargetAccountId,
        importDate: screenshotDate || getTodayIso(),
        totalCash: screenshotParseResult?.totalCash ?? null,
        positions: screenshotPositions,
        dryRun: false,
      });
      setScreenshotCommitResult(committed);
      await refreshPortfolioData();
    } catch (err) {
      setError(getParsedApiError(err));
    } finally {
  ...
  return (
    <div className="portfolio-page min-h-screen space-y-4 p-4 md:p-6">
      ...
      <Card className="border-cyan-400/40 shadow-[0_0_28px_rgba(0,212,255,0.10)]" padding="md">
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
            <div>
              <h2 className="text-base font-semibold text-foreground">截图导入持仓</h2>
              <p className="mt-1 text-xs text-secondary">
                上传同花顺/券商持仓截图，先识别预览，再确认写入。本功能只校准本地持仓，不会下单。
              </p>
            </div>
            <Badge variant={screenshotParseResult ? 'success' : screenshotParsing ? 'warning' : 'info'}>
              {screenshotParsing ? '识别中' : screenshotParseResult ? '需确认' : '待上传'}
            </Badge>
          </div>

          <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] gap-4">
            <div className="space-y-3">
              <label className="input-surface input-focus-glow flex min-h-[9rem] cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed px-4 py-5 text-center transition-all hover:border-cyan-300/60">
                <span className="text-sm font-semibold text-foreground">
                  {screenshotFile ? screenshotFile.name : '点击上传或拖入持仓截图'}
                </span>
                <span className="mt-2 text-xs text-secondary">支持 PNG / JPG；写入前必须人工确认预览表。</span>
                <input
                  aria-label="上传持仓截图"
                  type="file"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                <input
                  className={PORTFOLIO_INPUT_CLASS}
                  value={screenshotBrokerHint}
                  placeholder="券商提示，如 gf / ths"
                  onChange={(e) => setScreenshotBrokerHint(e.target.value)}
                />
                <input
                  className={PORTFOLIO_INPUT_CLASS}
                  type="date"
              ...
              <div className="flex flex-col gap-2 sm:flex-row">
                ...
                >
                  {screenshotParsing ? '识别中...' : '开始识别'}
                </button>
                <button
                  type="button"
                  className="btn-secondary flex-1"
                  disabled={!screenshotTargetAccountId || screenshotCommitting || screenshotPositions.length === 0 || hasInvalidScreenshotPositions}
                  onClick={() => void handleCommitScreenshot()}
                >
                  {screenshotCommitting ? '写入中...' : '确认写入'}
                </button>
              </div>
              <p className="text-[11px] text-secondary">
                写入目标：{screenshotTargetAccount ? `${screenshotTargetAccount.name} (#${screenshotTargetAccount.id})` : '请先创建或选择账户'}。
                写入后 report 将使用这份本地持仓。
              </p>
            </div>
            ...
            <div className="rounded-2xl border border-border/70 bg-elevated/80 p-3">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                <div>
                  ...
                  <p className="mt-1 font-semibold text-foreground">{screenshotParseResult?.suggestedAccountName || '未识别'}</p>
                </div>
                <div>
                  <p className="text-secondary">截图时间</p>
                  <p className="mt-1 font-semibold text-foreground">{screenshotParseResult?.screenshotDate || screenshotDate || '未识别'}</p>
                </div>
                <div>
                  <p className="text-secondary">识别行数</p>
                  <p className="mt-1 font-semibold text-foreground">{screenshotParseResult?.recordCount ?? screenshotPositions.length}</p>
                </div>
                <div>
                  <p className="text-secondary">现金</p>
                  <p className="mt-1 font-semibold text-foreground">{formatMoney(screenshotParseResult?.totalCash, 'CNY')}</p>
                </div>
              </div>
              <div className="mt-3 flex flex-wrap gap-2">
                {(screenshotParseResult?.warnings || []).slice(0, 3).map((warning) => (
                  <Badge key={warning} variant="warning">{warning}</Badge>
                ))}
                {hasInvalidScreenshotPositions ? <Badge variant="danger">有缺失字段</Badge> : null}
                {screenshotPositions.length > 0 && !hasInvalidScreenshotPositions ? <Badge variant="success">可写入</Badge> : null}
              </div>
              {screenshotImportWarning ? (
                <InlineAlert
                  variant="warning"
                  className="mt-3 rounded-lg px-3 py-2 text-xs shadow-none"
                  message={screenshotImportWarning}
                />
              ) : null}
              {screenshotCommitResult ? (
                <InlineAlert
                  variant={screenshotCommitResult.failedCount > 0 ? 'danger' : 'success'}
                  title="截图导入完成"
                  className="mt-3 rounded-lg px-3 py-2 text-xs shadow-none"
                  message={`写入 ${screenshotCommitResult.insertedCount} 条，重复 ${screenshotCommitResult.duplicateCount} 条，失败 ${screenshotCommitResult.failedCount} 条。`}
                />
              ) : null}
            </div>
          </div>
                    ...
                    <tr key={`${position.symbol}-${index}`} className="border-b border-border/50">
                      <td className="px-3 py-2">
                        <input
                          className={PORTFOLIO_INPUT_CLASS}
                          value={position.name || ''}
                          onChange={(e) => updateScreenshotPosition(index, 'name', e.target.value)}
                        />
                      </td>
                      <td className="px-3 py-2">
                        <input
                          className={`${PORTFOLIO_INPUT_CLASS} font-mono ${isScreenshotFieldWeak(position, 'symbol') ? 'border-yellow-300/70' : ''}`}
                          value={position.symbol}
                          onChange={(e) => updateScreenshotPosition(index, 'symbol', e.target.value)}
                        />
                      </td>
                      <td className="px-3 py-2">
                        <input
                          className={`${PORTFOLIO_INPUT_CLASS} text-right ${isScreenshotFieldWeak(position, 'quantity') ? 'border-yellow-300/70' : ''}`}
                          type="number"
                          min="0"
                          step="0.0001"
                          value={position.quantity}
                          onChange={(e) => updateScreenshotPosition(index, 'quantity', e.target.value)}
                        />
                      </td>
                      <td className="px-3 py-2">
                        <input
                          className="input-surface input-focus-glow h-11 w-full rounded-xl border bg-transparent px-3 text-right text-sm transition-all focus:outline-none"
                          type="number"
                          min="0"
                          step="0.0001"
                          value={position.availableQuantity ?? ''}
                          onChange={(e) => updateScreenshotPosition(index, 'availableQuantity', e.target.value)}
                        />
                      </td>
                      <td className="px-3 py-2">
                        <input
                          className={`${PORTFOLIO_INPUT_CLASS} text-right ${isScreenshotFieldWeak(position, 'avgCost') ? 'border-yellow-300/70' : ''}`}
                          type="number"
                          min="0"
                          step="0.0001"
                          value={position.avgCost}
                          onChange={(e) => updateScreenshotPosition(index, 'avgCost', e.target.value)}
                        />
                      </td>
                      <td className="px-3 py-2 text-right">{position.lastPrice != null ? position.lastPrice.toFixed(4) : '未识别'}</td>
                      <td className="px-3 py-2 text-right">{formatMoney(position.marketValue, 'CNY')}</td>
                      <td className="px-3 py-2 text-right">
                        <Badge variant={position.confidence >= 0.8 ? 'success' : 'warning'}>

...
function excludeMarketReviewItems(items: HistoryItem[]): HistoryItem[] {
  return items.filter((item) => item.stockCode !== MARKET_REVIEW_HISTORY_CODE && item.reportType !== 'market_review');
}

function selectPreferredInitialHistoryItem(items: HistoryItem[]): HistoryItem | undefined {
  return items.find((item) => item.hasMarketRadar) ?? items[0];
}
...
function canOpenStockHistoryTrend(report: AnalysisReport | null): boolean {
  const stockCode = (report?.meta.stockCode || '').trim().toUpperCase();
  if (!stockCode || stockCode === PORTFOLIO_DASHBOARD_CODE) {
    return false;
  }
  return true;
}
...
function reportToHistoryItem(report: AnalysisReport): HistoryItem | null {
  ...
  return {
    ...
    hasMarketRadar: Boolean(contextSnapshot?.marketRadar || contextSnapshot?.market_radar),
...
async function fetchStockHistory(
  ...
  if (!report || !canOpenStockHistoryTrend(report)) {
...
async function fetchHistory(
  ...
  try {
    ...
    const historyItems = excludeMarketReviewItems(response.items);
    ...
    if (silent && reset) {
      ...
      set({
        historyItems: [...get().historyItems, ...historyItems],
        currentPage: page,
      });
    }
    ...
    if (autoSelectFirst && historyItems.length > 0 && !get().selectedReport) {
      const preferredItem = selectPreferredInitialHistoryItem(historyItems);
      if (preferredItem) {
        await get().selectHistoryItem(preferredItem.id);
      }
    }
...
export const useStockPoolStore = create<StockPoolState>((set, get) => ({
  ...
  openHistoryTrend: async () => {
    if (!canOpenStockHistoryTrend(get().selectedReport)) {
      resetStockHistoryState(set);
  ...
  refreshTodayDashboard: async () => {
    const hasVisibleReport = Boolean(get().selectedReport);
    set({ isLoadingHistory: true, isLoadingReport: !hasVisibleReport, error: null });
    try {
      const report = normalizeSelectedReport(await historyApi.refreshTodayDashboard());
      const current = reportToHistoryItem(report);
      set({
        selectedReport: report,
        historyItems: current ? dedupeHistoryItems([current, ...get().historyItems]) : get().historyItems,
      });
    } catch (error) {
      set({ error: getParsedApiError(error) });
    } finally {
      set({ isLoadingHistory: false, isLoadingReport: false });
    }
  },
  ...
  selectHistoryItem: async (recordId) => {
    ...
    try {
      ...
      if (!canOpenStockHistoryTrend(report)) {
        stockHistoryRequestSeq += 1;
        resetStockHistoryState(set);
        set({ isHistoryTrendOpen: false });
        return;

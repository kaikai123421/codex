...
import type { AnalysisReport, TaskInfo, TaskListResponse } from '../../types/analysis';
...
describe('stockPoolStore', () => {
  ...
  it('keeps market radar dashboard reports in the history list without opening them on startup', async () => {
    const legacyItem = {
      ...historyItem,
      id: 1,
      queryId: 'legacy-report',
      hasMarketRadar: false,
    };
    const dashboardItem = {
      ...historyItem,
      id: 2,
      ...
      details: {
        contextSnapshot: {
          marketRadar: {
            date: '2026-07-03',
            cnMarket: {
              indices: [{ code: '000001', name: '上证指数', current: 4056.78 }],
            },
          },
        },
      },
    };
    vi.mocked(historyApi.refreshTodayDashboard).mockResolvedValue(dashboardReport);

    await useStockPoolStore.getState().refreshTodayDashboard();

    const state = useStockPoolStore.getState();
    expect(historyApi.refreshTodayDashboard).toHaveBeenCalledTimes(1);
    expect(state.selectedReport?.meta.queryId).toBe('today_market_dashboard_20260703');
    expect(state.historyItems[0].id).toBe(88);
    expect(state.historyItems[0].hasMarketRadar).toBe(true);
  });

  it('does not open same-stock trend history for the portfolio dashboard', async () => {
    const dashboardReport = {
      ...historyReport,
      meta: {
        ...historyReport.meta,
        ...
        queryId: 'today_market_dashboard_20260703',
        stockCode: 'PORTFOLIO',
        stockName: 'A-share portfolio',
      },
    };

    useStockPoolStore.setState({
      selectedReport: dashboardReport,
  ...
  it('keeps the visible report while refreshing today dashboard in the background', async () => {
    const deferred = createDeferred<AnalysisReport>();
    useStockPoolStore.setState({ selectedReport: historyReport, isLoadingReport: false });
    vi.mocked(historyApi.refreshTodayDashboard).mockReturnValue(deferred.promise);

    const refreshPromise = useStockPoolStore.getState().refreshTodayDashboard();

    expect(useStockPoolStore.getState().selectedReport?.meta.id).toBe(1);
    ...
    expect(useStockPoolStore.getState().selectedReport?.meta.id).toBe(89);
  });

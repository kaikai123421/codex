...
class DataFetcherManager:
    ...
    def get_chip_distribution(self, stock_code: str):
        ...
        fallback_chip = self._estimate_chip_distribution_from_daily(stock_code)
        if _is_meaningful_chip_distribution(fallback_chip):
            logger.info(
                "[筹码分布] %s 使用日线换手估算兜底成功 (source=%s)",
                stock_code,
                getattr(fallback_chip, "source", "daily_turnover_estimate"),
            )
    ...
    def _estimate_chip_distribution_from_daily(self, stock_code: str):
        """
        Estimate chip distribution from daily OHLCV when dedicated chip sources fail.

        This mirrors the common CYQ decay model used by public tooling: each day
        existing chips decay by turnover, while the day's turnover is distributed
        across the low-high range around the OHLC average. It is an approximation,
        but it keeps reports useful when Eastmoney/Tushare chip endpoints are
        unavailable.
        """
        from .realtime_types import ChipDistribution

        stock_code = normalize_stock_code(stock_code)
        if _is_us_market(stock_code) or _is_hk_market(stock_code) or _is_etf_code(stock_code):
            return None

        try:
            daily_df = None
            tencent = self._get_fetcher_by_name("TencentFetcher", capability="daily_data")
            if tencent is not None and hasattr(tencent, "get_daily_data"):
                try:
                    daily_df = tencent.get_daily_data(stock_code, days=210)
                except Exception as exc:
                    logger.debug("[筹码分布] %s 腾讯日线兜底失败，切换统一日线入口: %s", stock_code, exc)
            if daily_df is None or daily_df.empty:
                daily_result = self.get_daily_data(stock_code, days=210)
                daily_df = daily_result[0] if isinstance(daily_result, tuple) else daily_result
            if daily_df is None or daily_df.empty:
                return None

            quote = self.get_realtime_quote(stock_code)
            circulating_shares = self._infer_circulating_shares(daily_df, quote)
            if not circulating_shares or circulating_shares <= 0:
                logger.warning("[筹码分布] %s 无法估算流通股本，跳过日线兜底", stock_code)
                return None

            metrics = self._compute_cyq_metrics_from_daily(daily_df, circulating_shares)
            if not metrics:
                return None
            ...
            chip = ChipDistribution(
                code=stock_code,
                date=metrics["date"],
                source="daily_turnover_estimate",
            ...
            logger.warning("[筹码分布] %s 日线换手估算失败: %s", stock_code, exc)
            return None

    @staticmethod
    def _infer_circulating_shares(daily_df: pd.DataFrame, quote: Any) -> Optional[float]:
        price = _coerce_chip_metric(getattr(quote, "price", None))
        circ_mv = _coerce_chip_metric(getattr(quote, "circ_mv", None))
        if price and price > 0 and circ_mv and circ_mv > 0:
            return circ_mv / price

        turnover_rate = _coerce_chip_metric(getattr(quote, "turnover_rate", None))
        quote_volume = _coerce_chip_metric(getattr(quote, "volume", None))
        if turnover_rate and turnover_rate > 0 and quote_volume and quote_volume > 0:
            return quote_volume / (turnover_rate / 100.0)

        if "turnover_rate" in daily_df.columns and "volume" in daily_df.columns:
            valid = daily_df[["volume", "turnover_rate"]].dropna()
            valid = valid[(valid["volume"] > 0) & (valid["turnover_rate"] > 0)]
            if not valid.empty:
                latest = valid.iloc[-1]
                return float(latest["volume"]) / (float(latest["turnover_rate"]) / 100.0)

        return None

    @staticmethod
    def _compute_cyq_metrics_from_daily(
        ...
        *,
        ...
        if not required.issubset(set(daily_df.columns)):
            return None

        df = daily_df.sort_values("date").tail(210).copy()
        ...
        for index in range(len(records)):
            ...
            kdata = records[start : index + 1]
            max_price = max(float(item["high"]) for item in kdata)
            min_price = min(float(item["low"]) for item in kdata)
            if max_price <= 0 or min_price <= 0 or max_price < min_price:
                continue

            accuracy = max(0.01, (max_price - min_price) / (factor - 1))
            prices = np.array([min_price + accuracy * i for i in range(factor)], dtype=float)
            ...
            for item in kdata:
                ...
                volume = max(float(item["volume"] or 0), 0.0)
                turnover = min(1.0, max(0.0, volume / circulating_shares))
                chips *= 1.0 - turnover
                if turnover <= 0:
                    continue
                ...
                if high <= low:
                    pos = int(np.clip(np.floor((avg - min_price) / accuracy), 0, factor - 1))
                    chips[pos] += turnover
                    continue

                h_idx = int(np.clip(np.floor((high - min_price) / accuracy), 0, factor - 1))
                l_idx = int(np.clip(np.ceil((low - min_price) / accuracy), 0, factor - 1))
                if l_idx > h_idx:
                    continue

                weights = np.zeros(factor, dtype=float)
                for pos in range(l_idx, h_idx + 1):
                    cur_price = min_price + accuracy * pos
                    if cur_price <= avg:
                        denom = avg - low
                        weights[pos] = 1.0 if abs(denom) < 1e-8 else max((cur_price - low) / denom, 0.0)
                    else:
                        denom = high - avg
                        weights[pos] = 1.0 if abs(denom) < 1e-8 else max((high - cur_price) / denom, 0.0)
                total_weight = float(weights.sum())
                if total_weight > 0:
                    chips += weights / total_weight * turnover

            latest_distribution = chips
            latest_prices = prices

        if latest_distribution is None or latest_prices is None:
            return None

        total_chips = float(latest_distribution.sum())
        if total_chips <= 0:
            return None
        ...
        def cost_by_chip(target: float) -> float:
            cumulative = np.cumsum(latest_distribution)
            pos = int(np.searchsorted(cumulative, target, side="left"))
            pos = min(max(pos, 0), len(latest_prices) - 1)
            return float(latest_prices[pos])

        def percent_chips(percent: float) -> Tuple[float, float, float]:
            low_cost = cost_by_chip(total_chips * ((1.0 - percent) / 2.0))
            high_cost = cost_by_chip(total_chips * ((1.0 + percent) / 2.0))
        ...
        profit_ratio = float(latest_distribution[latest_prices <= current_price].sum() / total_chips)

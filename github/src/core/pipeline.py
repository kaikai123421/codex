...
class StockAnalysisPipeline:
    ...
    def _enhance_context(
        ...
        if trend_result:
            enhanced['trend_analysis'] = {
                ...
                'bbi': trend_result.bbi,
                'bias_ma5': trend_result.bias_ma5,
                'bias_ma10': trend_result.bias_ma10,
                'bias_ma20': trend_result.bias_ma20,
                'bias_bbi': trend_result.bias_bbi,
                'bbi_status': trend_result.bbi_status,
        ...
        if realtime_quote and trend_result and trend_result.ma5 > 0:
            ...
            if price is not None and price > 0:
                ...
                estimated_fields = [
                    'close', 'open', 'high', 'low', 'ma5', 'ma10', 'ma20', 'bbi',
                ]
                if vol is not None:
    ...
    def _build_context_snapshot(
        ...
        market_radar = self._build_market_radar_snapshot(market_phase_summary=market_phase_summary)
        if market_radar is not None:
            snapshot["market_radar"] = market_radar
        return snapshot

    def _build_market_radar_snapshot(
        self,
        *,
        market_phase_summary: Optional[Dict[str, Any]] = None,
    ) -> Optional[Dict[str, Any]]:
        """Best-effort A-share report dashboard payload.

        The payload never contains overseas trading actions. US/KR/JP markets are
        normalized to impact labels for A-share risk preference only.
        """
        try:
            radar_date = None
            if isinstance(market_phase_summary, dict):
                radar_date = (
                    market_phase_summary.get("effective_daily_bar_date")
                    or market_phase_summary.get("session_date")
                )
            return build_market_radar_payload(
                date=radar_date,
                data_manager=getattr(self, "data_manager", None),
            )
        except Exception as exc:
            logger.debug("market_radar snapshot skipped: %s", exc, exc_info=True)
            return None

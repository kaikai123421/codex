    ...
    def get_summary(
        ...
        *,
        ...
        if summary is None:
            return None
        if scope == "overall":
            visible_count = self.repo.count_results(
                code=code,
                eval_window_days=eval_window_days,
                engine_version=engine_version,
            ...
            if int(getattr(summary, "total_evaluations", 0) or 0) != int(visible_count):
                if visible_count > self.MAX_DYNAMIC_SUMMARY_ROWS:
                    raise ValueError(
                        "Overall summary source changed and matches too many rows; "
                        "narrow the evaluation window or date range."
                    )
                rows = self.repo.list_results(
                    code=code,
                    eval_window_days=eval_window_days,

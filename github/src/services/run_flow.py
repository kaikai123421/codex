...
_AUXILIARY_PROVIDER_DATA_TYPES = {"news", "news_search"}
...
def build_task_run_flow_snapshot(
    ...
    *,
    ...
    now_iso = _datetime_to_iso(generated_at_dt) or generated_at_dt.isoformat()
    elapsed_end = getattr(task, "completed_at", None)
    if elapsed_end is None and flow_status in {"pending", "running", "cancel_requested"}:
        elapsed_end = generated_at_dt
    task_elapsed_ms = _elapsed_ms(getattr(task, "created_at", None), elapsed_end)
...
def _append_provider_runs(
    ...
    for index, raw_run in enumerate(provider_runs, start=1):
        ...
        status = _provider_run_status(
            run,
            had_previous_failure=had_previous_failure,
            data_type=data_type,
        )
        ...
        severity = (
            ...
            if success
                ...
                "warning"
                if success_by_data_type.get(data_type) or data_type in _AUXILIARY_PROVIDER_DATA_TYPES
                else "danger"
            )
        )
        _append_event(

...
VIRTUAL_REPORT_CODES = {"PORTFOLIO", "MARKET"}
...
class HistoryService:
    ...
    @staticmethod
    def _has_market_radar(context_snapshot: Any) -> bool:
        snapshot_obj = parse_json_field(context_snapshot)
        if not isinstance(snapshot_obj, dict):
            return False
        candidate = snapshot_obj.get("market_radar") or snapshot_obj.get("marketRadar")
    ...
    def _record_to_list_item_dict(self, record) -> Dict[str, Any]:
        ...
        return {
            ...
            "has_market_radar": self._has_market_radar(getattr(record, "context_snapshot", None)),
    ...
    def resolve_and_get_news(self, record_id: str, limit: int = 20) -> List[Dict[str, str]]:
        ...
        try:
            ...
            if not record:
                ...
                return []
            if self._is_virtual_report_code(record.code):
                return []
    ...
    def _record_to_detail_dict(self, record) -> Dict[str, Any]:
        ...
        target_date = record.created_at.date() if getattr(record, "created_at", None) else None
        kline_chart = None
        if not self._is_virtual_report_code(record.code):
            kline_chart = build_report_kline_chart(str(record.code or ""), target_date=target_date)
        return {
            "id": record.id,
            "query_id": record.query_id,
            "stock_code": display_code,
    ...
    @staticmethod
    def _is_virtual_report_code(code: Any) -> bool:
        return str(code or "").strip().upper() in VIRTUAL_REPORT_CODES
        ...
        if data_persp:
            ...
            if price_data:
                ...
                report_lines.extend([
                    ...
                    f"| BBI趋势线 | {price_data.get('bbi', 'N/A')} {price_data.get('bbi_status', '')} |",
                    f"| {labels['bias_ma5_label']} | {price_data.get('bias_ma5', 'N/A')}% {bias_emoji}{bias_status} |",
                    f"| 价格位置(BBI) | {price_data.get('bias_bbi', 'N/A')}% |",

...
def _resolve_vision_model() -> str:
    ...
    cfg = get_config()
    # Prefer explicit vision settings only. Do not fall back to the primary
    # text model: providers such as DeepSeek may be configured for chat but
...
def _call_litellm_vision(
    ...
    *,
    ...
    if not model:
        raise ValueError("未配置 Vision API。请设置 VISION_MODEL，并配置对应视觉模型 API Key（推荐 GEMINI_API_KEY 或 OPENAI_API_KEY）。")

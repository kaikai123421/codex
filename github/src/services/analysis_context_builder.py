...
_A_SHARE_ETF_PREFIXES = ("51", "52", "56", "58", "15", "16", "18")
_CHIP_NOT_SUPPORTED_NAME_KEYWORDS = (
    "ETF",
    "LOF",
    "REIT",
    "基金",
...
def _build_chip_block(artifacts: PipelineAnalysisArtifacts) -> AnalysisContextBlock:
    ...
    if not chip:
        not_supported = _is_chip_distribution_not_supported(artifacts)
        status = (
...
def _is_chip_distribution_not_supported(
    artifacts: PipelineAnalysisArtifacts,
) -> bool:
    metadata = artifacts.metadata or {}
    if bool(metadata.get("chip_not_supported")):
        return True
    ...
    raw_code = str(artifacts.code or "").strip()
    code = raw_code.lower()
    if code.startswith(("sh", "sz")):
        code = code[2:]
    code = code.split(".")[0]
    if code.isdigit() and len(code) == 6 and code.startswith(_A_SHARE_ETF_PREFIXES):
        return True

    name = str(artifacts.stock_name or "")
    name_upper = name.upper()
    return any(keyword in name_upper for keyword in _CHIP_NOT_SUPPORTED_NAME_KEYWORDS)

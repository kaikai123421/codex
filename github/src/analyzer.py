...
def fill_price_position_if_needed(
    ...
    try:
        ...
        if trend_result:
            ...
            computed["bbi"] = tr.get("bbi")
            computed["bias_ma5"] = tr.get("bias_ma5")
            computed["bias_bbi"] = tr.get("bias_bbi")
            computed["bbi_status"] = tr.get("bbi_status")
    ...
    "dashboard": {
        ...
        "data_perspective": {
            ...
            "price_position": {
                ...
                "bbi": BBI趋势线数值,
                "bias_ma5": 乖离率百分比数值,
                "bias_bbi": BBI偏离率百分比数值,
                "bbi_status": "站上BBI/贴近BBI/跌破BBI/BBI未取得",
...
"""
        ...
        if 'trend_analysis' in context:
            ...
            bbi_value = _safe_float(trend.get('bbi'), default=math.nan)
            bbi_text = f"{bbi_value:.2f}" if math.isfinite(bbi_value) and bbi_value > 0 else unknown_text
            bias_bbi_value = _safe_float(trend.get('bias_bbi'), default=math.nan)
            bias_bbi_text = (
                f"{bias_bbi_value:+.2f}%"
                if math.isfinite(bias_bbi_value)
                else unknown_text
            )
            bbi_status = trend.get("bbi_status") or "BBI未取得"
            if use_legacy_default_prompt:
...
| **BBI趋势线** | **{bbi_text}** | {bbi_status} |
| **价格位置(BBI)** | **{bias_bbi_text}** | 长期趋势票必须结合日线/周线BBI判断 |
...
- **BBI趋势纪律**：问股技术面分析必须展示 BBI、价格位置(BBI) 与 BBI 状态；长期趋势票把日线/周线 BBI 作为趋势支撑/散户割肉线参考

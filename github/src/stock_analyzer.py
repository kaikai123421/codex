...
技术标准：
- 多头排列：MA5 > MA10 > MA20
- BBI 趋势线：(MA3 + MA6 + MA12 + MA24) / 4，长期趋势票必须参考
...
class TrendAnalysisResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            'bias_bbi': self.bias_bbi,
            'bbi_status': self.bbi_status,
...
class StockTrendAnalyzer:
    ...
    def analyze(self, df: pd.DataFrame, code: str) -> TrendAnalysisResult:
        ...
        latest_bbi = latest.get('BBI', 0)
        result.bbi = float(latest_bbi) if pd.notna(latest_bbi) and np.isfinite(latest_bbi) else 0.0
    ...
    def _calculate_bias(self, result: TrendAnalysisResult) -> None:
        ...
        if result.bbi > 0:
            result.bias_bbi = (price - result.bbi) / result.bbi * 100
            if abs(result.bias_bbi) <= 1:
                result.bbi_status = "贴近BBI"
            elif price > result.bbi:
                result.bbi_status = "站上BBI"
            else:
                result.bbi_status = "跌破BBI"
        else:
            result.bbi_status = "BBI未取得"

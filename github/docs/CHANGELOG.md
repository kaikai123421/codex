...
- [改进] A股持仓决策仪表盘打开时先展示最近可用 report，再后台刷新今日行情；收盘后生成的今日雷达会切换为“收盘后快照/收盘后雷达刷新”语境。
- [改进] A股持仓决策仪表盘刷新今日 `market_radar` 时标注行情新鲜度，并在指数/板块卡片展示来源、时间和滞后状态。
- [修复] A股雷达多源合并时保留复合来源，并优先使用最新东方财富资金流；同日缓存只补齐缺失的涨跌家数，避免旧缓存资金流覆盖新数据。

<!-- 新条目格式：- [类型] 描述（类型取值：新功能/改进/修复/文档/测试/chore）-->
<!-- 每条独立一行追加到本段末尾，无需分类标题，合并时冲突最小 -->

- [新功能] 新增 `stock_analyzer` 辅助用个人判断库样例规则，用于对用户给定标的结合长期投资原则、技术择时和风险纪律做可复盘判断。

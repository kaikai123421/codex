...
class RunFlowTestCase(unittest.TestCase):
    ...
    def test_active_task_elapsed_uses_generated_time_until_completion(self) -> None:
        task = TaskInfo(
            task_id="task-active-elapsed",
            trace_id="trace-active-elapsed",
            stock_code="600519",
            stock_name="贵州茅台",
        ...
        snapshot = build_task_run_flow_snapshot(
            task,
            generated_at=datetime(2026, 6, 8, 10, 0, 5),
        )
    ...
    def test_news_search_rate_limits_are_degraded_not_danger(self) -> None:
        context_snapshot = {
            "diagnostics": {
                "trace_id": "trace-news-rate-limit",
                "task_id": "task-news-rate-limit",
                "query_id": "query-news-rate-limit",
                "stock_code": "600519",
                "trigger_source": "api",
                "provider_runs": [
                    {
                        ...
                        "error_message_sanitized": "Too Many Requests",
                        "created_at": f"2026-07-02T23:25:0{index}",
                    }
                    for index, provider in enumerate(["SearXNG-A", "SearXNG-B", "SearXNG-C"], start=1)
                ],
                "llm_runs": [],
                "history_runs": [],
                "notification_runs": [],
        ...
        snapshot = build_history_run_flow_snapshot(_history_record(context_snapshot=context_snapshot))
        news_nodes = [node for node in snapshot.nodes if node.id.startswith("provider_news_search_")]
        news_events = [event for event in snapshot.events if event.type == "provider_run"]

        self.assertEqual(snapshot.status, "degraded")
        self.assertEqual({node.status for node in news_nodes}, {"degraded"})
        self.assertEqual({event.severity for event in news_events}, {"warning"})
        self.assertFalse(any(event.severity == "danger" for event in news_events))

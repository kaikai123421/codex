...
class StockAnalyzerBiasTestCase(unittest.TestCase):
    ...
    def test_analyze_calculates_bbi_and_bbi_bias(self) -> None:
        """Trend analysis should expose BBI and price distance from BBI."""
        ...
        result = self.analyzer.analyze(df, "000001")
        ...
        expected_bias = (closes[-1] - expected_bbi) / expected_bbi * 100

        self.assertAlmostEqual(result.bbi, expected_bbi)
        self.assertAlmostEqual(result.bias_bbi, expected_bias)
        self.assertEqual(result.bbi_status, "站上BBI")
        as_dict = result.to_dict()
        self.assertAlmostEqual(as_dict["bbi"], expected_bbi)
        self.assertAlmostEqual(as_dict["bias_bbi"], expected_bias)
        self.assertEqual(as_dict["bbi_status"], "站上BBI")

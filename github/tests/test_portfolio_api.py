...
class PortfolioApiTestCase(unittest.TestCase):
    ...
    def test_screenshot_import_parse_endpoint_returns_preview(self) -> None:
        parsed = {
            "suggested_account_name": "广发证券截图账户",
            "broker_hint": "gf",
            "screenshot_date": "2026-07-02",
            ...
            "positions": [
                {
                    ...
                    "last_price": 9.10,
                    "market_value": 5460.0,
                    "unrealized_pnl": 2645.91,
                    "unrealized_pnl_pct": 94.02,
                    "confidence": 0.96,
                    "field_confidence": {"symbol": 0.99, "quantity": 0.98, "avg_cost": 0.96},
                    "warnings": [],
                },
                {
                    "name": "通信ETF",
                    ...
                    "avg_cost": 0.954,
                    "last_price": 1.576,
                    "market_value": 13238.4,
                    "unrealized_pnl": 5226.33,
                    "unrealized_pnl_pct": 65.23,
                    "confidence": 0.93,
                    "field_confidence": {"symbol": 0.99, "quantity": 0.96, "avg_cost": 0.94},
                    "warnings": [],
                },
            ],
            "warnings": ["低置信度字段请确认后再写入"],
            "raw_summary": "识别到2条持仓。",
        }
        with patch(
            "api.v1.endpoints.portfolio.PortfolioImportService.parse_screenshot_import",
            return_value=parsed,
        ):
            resp = self.client.post(
                "/api/v1/portfolio/imports/screenshot/parse",
                data={"broker_hint": "gf", "screenshot_date": "2026-07-02"},
                files={"file": ("position.png", b"fake-image", "image/png")},
            )

        self.assertEqual(resp.status_code, 200)
        payload = resp.json()
        self.assertEqual(payload["suggested_account_name"], "广发证券截图账户")
        self.assertEqual(payload["record_count"], 2)
        self.assertEqual(payload["positions"][0]["symbol"], "000725")
        self.assertEqual(payload["positions"][0]["field_confidence"]["avg_cost"], 0.96)

    def test_screenshot_import_commit_writes_idempotent_calibration(self) -> None:
        account_resp = self.client.post(
            "/api/v1/portfolio/accounts",
            json={"name": "广发证券截图账户", "broker": "GF", "market": "cn", "base_currency": "CNY"},
        )
        ...
        payload = {
            "account_id": account_id,
            "import_date": "2026-07-02",
            "total_cash": 10896.04,
            "positions": [
                {"name": "京东方A", "symbol": "000725", "quantity": 600, "avg_cost": 4.69},
                {"name": "通信ETF", "symbol": "515880", "quantity": 8400, "avg_cost": 0.954},
            ],
        }

        first = self.client.post("/api/v1/portfolio/imports/screenshot/commit", json=payload)
        second = self.client.post("/api/v1/portfolio/imports/screenshot/commit", json=payload)

        self.assertEqual(first.status_code, 200)
        self.assertEqual(first.json()["inserted_count"], 3)
        self.assertEqual(first.json()["duplicate_count"], 0)
        self.assertEqual(second.status_code, 200)
        self.assertEqual(second.json()["inserted_count"], 0)
        self.assertEqual(second.json()["duplicate_count"], 3)
        ...
        account = snapshot.json()["accounts"][0]
        self.assertAlmostEqual(account["total_cash"], 10896.04, places=2)
        positions = {item["symbol"]: item for item in account["positions"]}
        self.assertEqual(positions["000725"]["quantity"], 600.0)
        self.assertEqual(positions["515880"]["quantity"], 8400.0)

    def test_screenshot_import_commit_rejects_missing_required_position_fields(self) -> None:
        payload = {
            "account_id": 1,
            "import_date": "2026-07-02",
            "positions": [{"name": "坏数据", "symbol": "", "quantity": 100, "avg_cost": 1.0}],
        }
        resp = self.client.post("/api/v1/portfolio/imports/screenshot/commit", json=payload)

...
class AnalysisHistoryTestCase(unittest.TestCase):
    ...
    def test_history_list_api_preserves_market_radar_marker(self, mock_auth) -> None:
        if TestClient is None or create_app is None:
            self.skipTest("fastapi is not installed in this test environment")

        result = self._build_result()
        result.code = "portfolio"
        result.name = "A股持仓决策仪表盘"
        saved = self.db.save_analysis_history(
            result=result,
            query_id="query_dashboard_market_radar",
            report_type="detailed",
            news_content="dashboard",
            context_snapshot={"market_radar": {"date": "2026-07-03", "cn_market": {}}},
            save_snapshot=True,
        )
        self.assertGreater(saved, 0)
        ...
        response = client.get("/api/v1/history?page=1&limit=5")

        self.assertEqual(response.status_code, 200)
        payload = response.json()
        item = next(
            entry for entry in payload["items"]
            if entry["query_id"] == "query_dashboard_market_radar"
        )
        self.assertTrue(item["has_market_radar"])

    @patch("src.auth.is_auth_enabled", return_value=False)
    def test_today_dashboard_refresh_api_saves_and_returns_market_radar_report(self, mock_auth) -> None:
        if TestClient is None or create_app is None:
            self.skipTest("fastapi is not installed in this test environment")

        def fake_save_today_dashboard(db_manager):
            result = self._build_result()
            result.code = "PORTFOLIO"
            result.name = "A股持仓组合"
            result.dashboard = {"market_radar": {"date": "2026-07-03"}}
            saved = db_manager.save_analysis_history(
                result=result,
                query_id="today_market_dashboard_20260703",
                report_type="detailed",
                news_content="dashboard",
                context_snapshot={
                    "market_radar": {
                        ...
                        "cn_market": {
                            "indices": [
                                {"code": "000001", "name": "上证指数", "current": 4056.78, "change_pct": 0.69}
                            ]
                        },
                    },
                    "marketRadar": {"date": "2026-07-03"},
                },
                save_snapshot=True,
            )
            return saved, None
        ...
        with patch("api.v1.endpoints.history.save_today_market_dashboard", side_effect=fake_save_today_dashboard):
            response = client.post("/api/v1/history/today-dashboard/refresh")

        self.assertEqual(response.status_code, 200)
        payload = response.json()
        self.assertEqual(payload["meta"]["query_id"], "today_market_dashboard_20260703")
        self.assertEqual(payload["details"]["context_snapshot"]["market_radar"]["date"], "2026-07-03")
        self.assertEqual(
            payload["details"]["context_snapshot"]["market_radar"]["cn_market"]["indices"][0]["current"],
            4056.78,
        )
    ...
    def test_history_detail_exposes_kline_chart_from_daily_history(self) -> None:
        """History detail should include real daily K-line bars and BBI for the report chart."""
        if get_history_detail is None:
            self.skipTest("fastapi is not installed in this test environment")
        ...
        for index in range(32):
            ...
            rows.append(
                {
                    "date": end_date - timedelta(days=31 - index),
                    "open": close - 1,
                    "high": close + 2,
                    "low": close - 2,
                    "close": close,
        ...
        self.db.save_daily_data(pd.DataFrame(rows), "600519", "unit_test")

        record_id = self._save_history("query_kline_chart_001")

        report = get_history_detail(str(record_id), db_manager=self.db)

        self.assertIsNotNone(report.details)
        self.assertIsNotNone(report.details.kline_chart)
        chart = report.details.kline_chart
        self.assertEqual(chart["source"], "db_cache")
        self.assertEqual(len(chart["bars"]), 32)
        latest_bar = chart["bars"][-1]
        self.assertEqual(latest_bar["date"], end_date.isoformat())
        self.assertEqual(latest_bar["close"], 131.0)
        self.assertIsNotNone(latest_bar["ma5"])
        self.assertIsNotNone(latest_bar["ma20"])
        self.assertIsNotNone(latest_bar["bbi"])

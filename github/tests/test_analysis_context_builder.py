...
def test_chip_missing_for_etf_is_not_supported_without_explicit_metadata() -> None:
    pack = AnalysisContextBuilder.build(
        _artifacts(
            code="515880",
            stock_name="通信ETF国泰",
    ...
    block = pack.blocks["chip"]
    assert block.status == ContextFieldStatus.NOT_SUPPORTED
    assert block.items["chip_distribution"].missing_reason == "chip_not_supported"

...
class BacktestServiceTestCase(unittest.TestCase):
    ...
    def test_run_backtest_excludes_portfolio_review_records(self) -> None:
        with self.db.get_session() as session:
            session.add(
                AnalysisHistory(
                    query_id="q-portfolio-review",
                    code="PORTFOLIO",
                    name="A股持仓组合",
                    ...
                    analysis_summary="portfolio review summary",
                    stop_loss=None,
                    take_profit=None,
                    created_at=datetime(2024, 1, 3, 0, 0, 0),
                    context_snapshot='{"enhanced_context": {"date": "2024-01-03"}}',
        ...
        with self.db.get_session() as session:
            self.assertEqual(
                session.query(BacktestResult).filter(BacktestResult.code == "PORTFOLIO").count(),
                0,
            )

    def test_recent_evaluations_hide_legacy_portfolio_results(self) -> None:
        with self.db.get_session() as session:
            portfolio_history = AnalysisHistory(
                query_id="q-legacy-portfolio-result",
                code="PORTFOLIO",
                name="A股持仓组合",
                report_type="portfolio",
                sentiment_score=50,
                operation_advice="等待持仓截图校准后决策",
                trend_prediction="盘中雷达刷新",
                analysis_summary="legacy portfolio review summary",
                stop_loss=None,
                take_profit=None,
                created_at=datetime(2024, 1, 3, 0, 0, 0),
                context_snapshot='{"enhanced_context": {"date": "2024-01-03"}}',
            )
            stock_history = AnalysisHistory(
                query_id="q-legacy-stock-result",
                code="000858",
                name="五粮液",
                report_type=None,
                sentiment_score=60,
                operation_advice="持有",
                trend_prediction="震荡",
                analysis_summary="legacy stock summary",
                stop_loss=None,
                take_profit=None,
                created_at=datetime(2024, 1, 3, 0, 0, 0),
                context_snapshot='{"enhanced_context": {"date": "2024-01-03"}}',
            )
            session.add_all([portfolio_history, stock_history])
            session.flush()
            session.add_all(
                [
                    BacktestResult(
                        analysis_history_id=portfolio_history.id,
                        code="PORTFOLIO",
                        analysis_date=date(2024, 1, 3),
                        eval_window_days=1,
                        engine_version="v1",
                        eval_status="insufficient_data",
                        evaluated_at=datetime(2024, 1, 4, 0, 0, 0),
                        operation_advice="等待持仓截图校准后决策",
                    ),
                    BacktestResult(
                        analysis_history_id=stock_history.id,
                        code="000858",
                        analysis_date=date(2024, 1, 3),
                        eval_window_days=1,
                        engine_version="v1",
        ...
        service = BacktestService(self.db)
        data = service.get_recent_evaluations(code=None, eval_window_days=1, limit=10, page=1)

        self.assertEqual(data["total"], 1)
        self.assertEqual([item["code"] for item in data["items"]], ["000858"])

    def test_overall_summary_recomputes_when_legacy_portfolio_results_are_hidden(self) -> None:
        with self.db.get_session() as session:
            portfolio_history = AnalysisHistory(
                query_id="q-legacy-portfolio-summary",
                code="PORTFOLIO",
                name="A股持仓组合",
                report_type="portfolio",
                sentiment_score=50,
                operation_advice="等待持仓截图校准后决策",
                trend_prediction="盘中雷达刷新",
                analysis_summary="legacy portfolio summary",
                stop_loss=None,
                take_profit=None,
                created_at=datetime(2024, 1, 3, 0, 0, 0),
                context_snapshot='{"enhanced_context": {"date": "2024-01-03"}}',
            ...
            stock_history = AnalysisHistory(
                query_id="q-valid-stock-summary",
                code="000858",
                name="五粮液",
                report_type=None,
                sentiment_score=60,
                operation_advice="持有",
                trend_prediction="震荡",
                analysis_summary="valid stock summary",
                stop_loss=None,
                take_profit=None,
                created_at=datetime(2024, 1, 3, 0, 0, 0),
                context_snapshot='{"enhanced_context": {"date": "2024-01-03"}}',
        ...
        service = BacktestService(self.db)
        summary = service.get_summary(scope="overall", code=None, eval_window_days=1)

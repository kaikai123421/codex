...
class AnalyzerNewsPromptTestCase(unittest.TestCase):
    ...
    def test_format_prompt_requires_bbi_in_technical_analysis(self) -> None:
        with patch.object(GeminiAnalyzer, "_init_litellm", return_value=None):
            analyzer = GeminiAnalyzer(
                skill_instructions="### 技能 1: 趋势交易\n- 长期趋势票看 BBI",
                default_skill_policy="",
                use_legacy_default_prompt=False,
            )
        ...
        self.assertIn("问股技术面分析必须展示 BBI", prompt)
        self.assertIn("价格位置(BBI)", prompt)
        self.assertIn("95.50", prompt)
        self.assertIn("+4.71%", prompt)
        self.assertIn("站上BBI", prompt)
